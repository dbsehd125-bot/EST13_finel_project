  ...
  useEffect(() => {
    ...
    const fetchAiStepSummaries = async () => {
      try {
         ...
         */
        const { data: savedSummaries, error: saveError } = await supabase.rpc(
          "complete_recipe_ai_summary",
          {
            target_recipe_id: recipe.id,
         ...
         * DB가 반환한 값을 우선 사용
         */
        const finalSummaries = hasAiSummaries(savedSummaries)
          ? savedSummaries
          : normalizedSummaries;
